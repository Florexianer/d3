<template>
  ...
  <div style="display:inline-block;">
    <div style="height: 32px;">
      <div @click.prevent="drawStandardGraph" style="cursor:pointer; display:flex; align-items:center; justify-content: start" v-if="detail">
    ...
    <div style="padding: 15px;border: solid black; display: inline-block">
      <div id="chart"></div>
...
// Add the X and Y axes
    ...
    drawUserGraph(data, groupedData) {
      ...
      // Iterate through the user names and create a checkbox input element with a label for each user
      const uniqueUsers = [...new Set(data.map(d => d.user))];
      uniqueUsers.forEach(user => {
        const checkbox = checkboxesDiv.append("input")
            .attr("type", "checkbox")
            .attr("id", "checkbox-" + user)
            .attr("value", user)
            .property("checked", true);

        checkboxesDiv.append("label")
            .attr("for", "checkbox-" + user)
            .text(user);

        // Add a change event listener to each checkbox to update the graph based on the user's selection

        checkbox.on("change", () => this.updateUserGraph(data, groupedData, checkboxesDiv));
      })
      ...
      const userGraphMargin = { top: 20, right: 80, bottom: 70, left: 80 };

      const userSvg = d3.select("#chart")
          .append("svg")
    ...
    updateUserGraph(data, groupedData, checkboxesDiv) {
      d3.select("svg").remove();
      const selectedUsers = checkboxesDiv.selectAll("input:checked").nodes().map(node => node.value);

      data = data.filter(d => selectedUsers.includes(d.user));
